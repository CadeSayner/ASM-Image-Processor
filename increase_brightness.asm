...
main:
    ...
    li  $v0, 13     # System call to open file, $v0 set to file descriptor
    la  $a0, output_file_location   # Load file to read, $a0 set to address of string
    li  $a1, 1     
    li  $a2, 0
    syscall

    add $s1, $v0, $zero # Save file descriptor in $v0 to new register $s
    blt $v0, 0, error 
    li $t7, 0 # Initialise counter at 0

write_headers_to_output_buffer: # write image headers to output_buffer
    lb $t2, header($t7)
    beq $t2, 0, read
...
read:
    ...
    lb $t1, 7($t0)
    sb $t1, 7($t2)
    li $t0, 19 # Start of first number
...
loop:
    li $t4, 0 # t4 stores current integer value 
    move $t5, $t0, # What $t0 was at the beginning of this iteration
    jal get_next_integer
    # If here succesfully got the integer plus ten in $t4 and $t0 is currently at a newline character in buffer
    ...
    sb $t1, output_buffer($t0) # Add the linebreak to the end of the number
...
integer_to_ascii:
    # This code loads the correct ascii representation for the integer in $t4 and places it into stack memory for later reading
...
get_next_integer: # reads the next integer, converting from its ascii form
    lb $t2, buffer($t0)
    beq $t2, 10, get_next_integer_completed
    beq $t2, 0, main_loop_done
    addi $t2, $t2, -48 # To get the digits actual value
...
overflow: # To avoid going over a pixel value of 255
    li $t4, 255
    j $ra

write_to_file:
    li   $v0, 15       # system call for write to file
    move $a0, $s1      # file descriptor, saved when we opened the file in main
    la   $a1, output_buffer   # address of buffer from which to write
    move  $a2, $s2    # hardcoded buffer length
    syscall            # write to file
    # Close the file now 
    li $v0, 16
    syscall

print_averages: # Code responsible for printing the average pixel values both before and after the brightness increase 
    mtc1.d $s7, $f12
    cvt.d.w $f12, $f12
    li $v0, 4
    la $a0, output_average_before_message # Print the message
    syscall
    li $t0, 3133440
    mtc1.d $t0, $f14
    cvt.d.w $f14, $f14
    div.d $f12, $f12, $f14
    li $v0, 3 # Print the average stored in the floating point register
    syscall
    ...
    li $v0, 4
    la $a0, output_average_after_message
    syscall
...
error:
    li $v0, 4    
    la $a0, exception # Output exception message to the console
    syscall

main_loop_done:
    j write_to_file # Main loop done, can write to the file now
